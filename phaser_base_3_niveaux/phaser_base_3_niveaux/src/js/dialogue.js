...
class Dialogue extends Phaser.Scene {
    constructor() {
        super({ key: 'Dialogue' }); // Initialisation de la scène avec un identifiant
    }

    preload() {
        // Chargement des ressources nécessaires pour la scène
        ...
        this.load.image("alcohol_bottle1", "src/assets/alcohol_bottle1.png");
        ...
        this.load.image("bouton_n2", "src/assets/porten2.png"); // Bouton pour passer au niveau 2
    }
    ...
    create() {
        ...
        this.personnage1.setCollideWorldBounds(true); // Empêche le personnage de sortir de l'écran

        this.personnage2 = this.physics.add.sprite(650, 350, 'personnage2');
        this.personnage2.setBounce(0.2);
        this.personnage2.setCollideWorldBounds(true); // Empêche le personnage de sortir de l'écran

        // Gérer les collisions entre les personnages et le bar
        ...
        this.input.keyboard.on('keydown-SPACE', () => {
            ...
            if (this.currentLineIndex < this.dialogueLines.length) {
                this.displayDialogue(); // Afficher la ligne suivante
            } else {
        ...
        // Ajouter un effet de survol pour le bouton niveau 2
    ...
    // Fonction pour afficher le dialogue en fonction de l'index actuel
    displayDialogue() {
        const line = this.dialogueLines[this.currentLineIndex];
        if (line.character === 1) {
            this.dialogueText1.setText(line.text); // Afficher le texte du personnage 1
            this.dialogueText2.setText(''); // Effacer le texte du personnage 2
        } else {
            this.dialogueText2.setText(line.text); // Afficher le texte du personnage 2
            this.dialogueText1.setText(''); // Effacer le texte du personnage 1
        }
    }
}

export default Dialogue; // Exporter la classe pour l'utiliser dans d'autres fichiers
